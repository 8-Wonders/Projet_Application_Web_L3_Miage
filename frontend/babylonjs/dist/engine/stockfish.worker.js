...
self.onmessage = function(e) {
  try {
    if (e.data.cmd === "load") {
      ...
      } else if (e.data.urlOrBlob && typeof URL !== "undefined") {
        var objectUrl = URL.createObjectURL(e.data.urlOrBlob);
        importScripts(objectUrl);
        URL.revokeObjectURL(objectUrl);
      } else {
        // Fallback: If no URL provided, try to load stockfish.js from relative path
    ...
    } else if (e.data.target === "setimmediate") {} else if (e.data.cmd === "check-interactive") {} else if (e.data.cmd === "custom") {
        if (typeof Module["onCustomMessage"] === "function") {
            Module["onCustomMessage"](e.data.userData);
        }
    } else {
      err("worker.js onmessage() captured an uncaught exception: " + e.data);
    }
  } catch (ex) {
